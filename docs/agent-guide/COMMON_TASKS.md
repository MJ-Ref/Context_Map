...
1. **Check for an existing plan.** Look in `plans/active/` for a plan covering this feature. If none exists, create one first (see "Creating an Execution Plan" below).
...
6. **Move the plan.** Move the plan from `plans/active/` to `plans/completed/`. Update `plans/_INDEX.md`.
...
1. **Copy the template.** Copy `plans/_TEMPLATE.md` to `plans/active/` with a descriptive filename (e.g., `add-auth-module.md`).
...
4. **Update the index.** Add an entry to `plans/_INDEX.md` with the plan name, status, and one-line description.
...
1. **Create the ADR file.** Use the next sequential number: `docs/architecture/ADR/NNN-short-title.md`. Check existing files in `docs/architecture/ADR/` for the latest number.
...
4. **Commit.** Commit with a message like `docs: ADR-NNN short description`.
...
2. **Freshness check.**
   ...
   ```
   Reports documents with `<!-- reviewed: -->` dates older than 30 days.
...
| Task | Key Files |
|---|---|
| New feature | `plans/active/`, source files, `plans/_INDEX.md` |
